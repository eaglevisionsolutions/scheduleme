...
class SCME_Admin_Settings {
    ...
    public function register_settings() {
        ...
        register_setting('SCME_settings_group', 'SCME_google_access_token');
        register_setting('SCME_settings_group', 'SCME_google_refresh_token');

        // Register PayPal Settings
        register_setting('SCME_settings_group', 'SCME_paypal_email');
        register_setting('SCME_settings_group', 'SCME_paypal_sandbox_mode');
        register_setting('SCME_settings_group', 'SCME_paypal_ipn_url');
    ...
    public function ipn_url_callback() {
        $ipn_url = home_url('/your-custom-booking-paypal-listener/');
    ...
    public function settings_page_content() {
        ...
        <div class="wrap">
            ...
            <hr>
            <?php
            $google_connected = get_option('SCME_google_access_token');
            if (!$google_connected) {
                ?>
                <h2><?php esc_html_e('Manual Availability', 'your-custom-booking-plugin'); ?></h2>
                <form method="post" action="options.php">
                    <?php
                    settings_fields('scme_availability_group');
                    do_settings_sections('scme-availability');
                    submit_button();
                    ?>
                </form>
                <?php
            } else {
                echo '<p><strong>Availability is managed via Google Calendar integration.</strong></p>';
            }
            ?>
        </div>
        <?php
